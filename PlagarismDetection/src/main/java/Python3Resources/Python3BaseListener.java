...
public class Python3BaseListener implements Python3Listener {
	 ...
	 * List of tree nodes.
	 */
	private List<List<Integer>> treeMap = new ArrayList<>();
	...
	/**
	 * Visits node and get line numbers of every node
	 * @param node
	 */
	@Override public void visitTerminal(TerminalNode node) {
		List<Integer> astNodeAndLine = new ArrayList<>();
		astNodeAndLine.add(node.getSymbol().getType());
		astNodeAndLine.add(node.getSymbol().getLine());
		treeMap.add(astNodeAndLine);
	}
